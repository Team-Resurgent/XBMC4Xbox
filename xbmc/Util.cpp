...
void CUtil::RemoveIllegalChars( CStdString& strText)
{
  ...
  static char legalChars[] = " ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890!#$%&'()-@[]^_`{}~.ﬂÂƒ‰÷ˆ¸¯ÈËÁ‡˘Í¬Ò·ÔÎÏÌ‚„ÊÓÚÙÛı˜˙";
  ...
  for (cursor = szRemoveIllegal; *(cursor += strspn(cursor, legalChars)); /**/ )
  {
    ...
    if (strchr("¬¡¿ƒ√≈", (int) *cursor)) *cursor = 'A';
    else
    if (strchr("‚·‡‰„Â", (int) *cursor)) *cursor = 'a';
    else
    if (strchr("‘”“÷’", (int) *cursor)) *cursor = 'O';
    else
    if (strchr("ÙÛÚˆı", (int) *cursor)) *cursor = 'o';
    else
    if (strchr("€⁄Ÿ‹", (int) *cursor)) *cursor = 'U';
    else
    if (strchr("˚˙˘¸µ", (int) *cursor)) *cursor = 'u';
    else
    if (strchr(" …»À", (int) *cursor)) *cursor = 'E';
    else
    if (strchr("ÍÈËÎ", (int) *cursor)) *cursor = 'e';
    else
    if (strchr("ŒÕÃœ", (int) *cursor)) *cursor = 'I';
    else
    if (strchr("ÓÏÌÔ", (int) *cursor)) *cursor = 'i';
    else
    *cursor = '_';
  }
    ...
    *szBackslash = 0x00;
    ...
    if (szColon)
    {
      *szColon = 0x00;
      ...
      CIoSupport::GetPartition(szDrive, szDevicePath);

      strcat(szDevicePath, szDirectory);
      wsprintf(szXbePath, "DVD-ROM:\\%s", szXbe);
...
void CUtil::LaunchXbe(const char* szPath, const char* szXbe, const char* szParameters, F_VIDEO ForceVideo, F_COUNTRY ForceCountry, CUSTOM_LAUNCH_DATA* pData)
{
...
#ifdef HAS_XBOX_HARDWARE
  CIoSupport::RemapDriveLetter("DVD-ROM", const_cast<char*>(strPath.c_str()));
  ...
  if (pData)
  {
    ...
    CIoSupport::UnmapDriveLetter("DVD-ROM");
