...
//*************************************************************************************************************
CSmartXXLCD::CSmartXXLCD(unsigned int type)
{
    m_iType = type;
    m_iActualpos = 0;
    m_iRows = 4;
...
void CSmartXXLCD::DisplayBuildCustomChars()
{
    ...
    {
        for (I = 0; I < 64; I++)
            DisplayOut(GetLCDCharsetCharacter(I), DAT); // all numberblocks chars
    }
    else
    {
        for (I = 0; I < 8; I++)
            DisplayOut(Bar0[I], DAT); // Bar0
        for (I = 0; I < 8; I++)
            DisplayOut(Bar1[I], DAT); // Bar1
        for (I = 0; I < 8; I++)
            DisplayOut(Bar2[I], DAT); // Bar2
        for (I = 0; I < 8; I++)
            DisplayOut(REW[AnimIndex][I], DAT); // REW
        for (I = 0; I < 8; I++)
            DisplayOut(FF[AnimIndex][I], DAT); // FF
        for (I = 0; I < 8; I++)
            DisplayOut(Play[I], DAT); // Play
        // for(I=0;I<8;I++) DisplayOut(Stop[I], DAT);  // Stop - dont need as its a built in char
        for (I = 0; I < 8; I++)
            DisplayOut(Bar3[I], DAT);
        for (I = 0; I < 8; I++)
            DisplayOut(Pause[I], DAT); // Pause
    }
...
{
    if ((m_iType & LCD_TYPE_VFD) == LCD_TYPE_VFD)
    {
